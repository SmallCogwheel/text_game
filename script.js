...
async function selectCharacter(className) {
    ...
    updateInventoryUI(); // [ITEM] 인벤토리 UI에 시작 아이템 즉시 출력!
    renderMap();         // 맵 렌더링
}
...
function restRoom(message = "") {
    ...
    story.innerHTML = `
        ...
        <span style="color: #ffaa00; font-weight: bold;">현재 층: ${currentFloor} / ${maxFloor} | 보유 골드: [G] ${player.gold} | 보유 은화: ${player.silver} | 이번 층 무료 충전 기회: ${remainingRest} / ${maxRestCount}회</span><br><br>
    `;

    if (remainingRest > 0) {
        story.innerHTML += `<button onclick="useRestStation()">[+] 무료 에너지 충전하기</button>`;
    } else {
        story.innerHTML += `<span style="color: #ff3333; font-weight: bold;">[ERR] 발전기 코어가 과열되어 더 이상 에너지를 공급받을 수 없습니다!</span>`;
    }
...
function spendGold(cost) {
    if (player.gold < cost) {
        restRoom(`[G] 골드가 부족합니다. 필요한 골드: ${cost}`);
        return false;
    }
...
function buyShopHeal() {
    ...
    updateStats();
    restRoom(`[HP] 응급 수리 완료! HP가 ${healAmount} 회복되었습니다.`);
}

function buyShopScan() {
    if (totalSearches <= 0) {
        restRoom("[SCN] 조사 횟수가 이미 가득 차 있습니다.");
        return;
    }
...
function encounterEnemy(key, isNew = true) {
    ...
    if (isNew) {
        if (currentEnemy.isElite) {
            ...
            story.innerHTML = `[BOT] 기계 경보 발령! <b>${currentEnemy.name}</b>이(가) 나타났다!`;
        }
    }
    renderMap(); 
    renderBattle();
...
function renderBattle() {
    ...
    choicesDiv.innerHTML = `
        <button onclick="attack()">[ATK] 공격하기</button>
        <button onclick="runAway()">[RUN] 도망치기</button>
    `;
}
...
function attack() {
    ...
    let log = `[ATK] ${currentEnemy.name}에게 ${baseDamage}의 피해를 주었습니다.<br>`;
    ...
    log += `[HIT] ${currentEnemy.name}이(가) 반격하여 ${damageTaken}의 피해를 입었습니다. (방어력으로 ${blockedDamage} 감소)`;
...
function runAway() {
    ...
    story.innerHTML = `[RUN] 몬스터를 피해 직전에 있었던 안전한 방 (${player.x}, ${player.y})으로 황급히 텔레포트하여 도망쳤습니다! <br>몬스터의 체력 상태는 그대로 유지됩니다.`;
    inBattle = false;
    currentEnemy = null;
    currentRoomKey = null; 
    ...
    if (currentEnemy.id === "core_gear") {
        if (currentFloor < maxFloor) {
            story.innerHTML = `
                ...
                획득 골드: [G] ${currentEnemy.gold}<br>
                현재 골드: [G] ${player.gold}<br><br>
    ...
    if (currentEnemy.isElite) {
        ...
        for (let i = 1; i <= 2; i++) {
            ...
            if (randomItem) {
                ...
                eliteRewardLog += `<br>[MOD] <b>${randomItem.name}</b> 획득! <span style="color: #62ff62;">(${logEffects})</span>`;
            }
        }
    }

    const rewardText = currentEnemy.isElite ? `[HOT] [정예 처치] [G] ${currentEnemy.gold}` : `[G] ${currentEnemy.gold}`;
    
    story.innerHTML += `
        <br>[WIN] 승리! ${currentEnemy.name}를 처치하고 ${rewardText} 골드를 획득했습니다.${eliteRewardLog}
...
function ascendToNextFloor() {
    ...
    story.innerHTML = `
        ...
        <span style="color: #62ff62;">[MED] 층 이동으로 내구도가 완전히 회복되었습니다! (HP: ${player.maxHp}/${player.maxHp})</span><br><br>
...
// 시체 루팅 (메카닉 특전 반영)
function lootCorpse() {
    // [ENG] 엔지니어(mechanic)인 경우 트랩을 무조건 무력화하고 100% 확률로 성공!
    if (chosenClass === "mechanic") {
        const subChance = Math.random();
        if (subChance < 0.40) {
            const extraGold = Math.floor(Math.random() * 15) + 5;
            player.gold += extraGold;
            story.innerHTML = `[ENG] <b>[ENGINEER_PRIVILEGE] 시체 정밀 해체 성공!</b><br><br>기계 전문가의 솜씨로 안전하게 내부 비상 금고를 우회 해킹했습니다!<br><span style="color: #ffff44;">[G] 추가 골드 +${extraGold} 획득!</span>`;
        } else {
            const item = getRandomItem();
            applyItemReward(item, false);
            const logEffects = formatItemEffects(item);
    ...
    // 그 외 다른 클래스들은 대부분 잔해만 뒤지고, 낮은 확률로 보상/자폭/대폭발이 발생합니다.
        ...
        if (randomChance < LOOT_SUCCESS_CHANCE) {
            ...
            if (subChance < LOOT_GOLD_CHANCE) {
                const extraGold = Math.floor(Math.random() * 15) + 5;
                player.gold += extraGold;
                story.innerHTML = `[DEAD] <b>시체 수색 성공!</b><br><br>숨겨진 내부 비상 금고를 해킹하여 대박을 터트렸습니다!<br><span style="color: #ffff44;">[G] 추가 골드 +${extraGold} 획득!</span>`;
            } else {
                const item = getRandomItem();
                applyItemReward(item, false);
                const logEffects = formatItemEffects(item);
            ...
            if (isMegaExplosion) {
                story.innerHTML = `
                    ...
                    [BOOM] 대폭발로 인해 <span style="color: #ff3333; font-weight: bold;">체력이 -${trapDamage}</span> 깎였습니다.<br>
                ...
                story.innerHTML = `
                    ...
                    [HIT] 쿠우웅! 폭발로 인해 <span style="color: #ff3333; font-weight: bold;">체력이 -${trapDamage}</span> 깎였습니다.<br>
...
function searchRoom() {
    if (inBattle || isGameOver) {
        story.innerHTML = `[ATK] 전투 중에는 조사할 수 없다.`;
        return;
    }

    if (totalSearches >= maxSearches) {
        story.innerHTML = `[SCN] 더 이상 조사할 수 없다.<br><br>모든 조사 기회를 사용했다.`;
        return;
    }
    ...
    if (!item) {
        story.innerHTML = `[SCN] 방을 샅샅이 뒤졌지만 쓸만한 고철을 찾지 못했습니다.`;
        updateStats();
        return;
    }
